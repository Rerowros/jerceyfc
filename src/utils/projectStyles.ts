...
export function getProjectStyle(tags: string[], type: string): ProjectStyle {
  ...
  if ((tagSet.has("Next.js") || tagSet.has("React")) && (tagSet.has("Crypto") || tagSet.has("Web3"))) {
    return { 
  ...
  for (const tag of tags) {
    switch (tag) {
      ...
      case "Python":
        return { gradient: "from-blue-900 via-slate-800 to-yellow-900", iconColor: "text-yellow-400", label: "PYTHON" };
      case "C#":
      case "WinUI 3":
        return { gradient: GRADIENTS.purple, iconColor: "text-purple-400", label: "C#" };
      case "Go":
        return { gradient: "from-cyan-900 via-blue-900 to-slate-900", iconColor: "text-cyan-300", label: "GO" };
      case "Rust":
        return { gradient: "from-orange-900 via-red-900 to-slate-900", iconColor: "text-orange-400", label: "RUST" };
      case "Docker":
      case "DevOps":
        return { gradient: "from-blue-900 via-sky-900 to-slate-900", iconColor: "text-blue-300", label: "DEVOPS" };
    }
  }
